...
| `cpu_process`     | This JVM process's share of CPU averaged over the last minute (0.0 to 1.0). When close to `cpu`, Minecraft is the dominant load; when much smaller, something else on the host is competing. May transiently exceed `cpu` due to sampling skew between the two values; this is a known artifact, not a measurement error. |
